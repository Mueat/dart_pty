...
class _NamedPipe {
  _NamedPipe({bool nowait = false}) {
    const pipeName = r'\\.\pipe\dart-pty-pipe';
...
class WinPty implements PseudoTerminal {
  WinPty({
    ...
    final pipe2 = win32.CreatePipe(hReadPipe, hWritePipe, nullptr, 0);
    ...
    // final oReadPipe = calloc<IntPtr>();
    // final oWritePipe = calloc<IntPtr>();
    // final pipe1 = win32.CreatePipe(oReadPipe, oWritePipe, nullptr, 0);
    // final outputReadSide = oReadPipe.value;
    // final outputWriteSide = oWritePipe.value;
  ...
  @override
  bool close([ProcessSignal signal = ProcessSignal.sigterm]) {
  ...
  Uint8List? readSync() {
    ...
    // win32.ReadConsole(hConsoleInput, lpBuffer, nNumberOfCharsToRead, lpNumberOfCharsRead, pInputControl)
    // win32.ReadFile(hFile, lpBuffer, nNumberOfBytesToRead, lpNumberOfBytesRead, lpOverlapped)
  ...
  Future<void> _startPolling() async {
    ...
    if (addV.isNotEmpty) {
      _out.sink.add(utf8.decode(addV));
    }
